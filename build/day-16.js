...
let  solutionOneEntryPoints  =  [{ originDirection: `RIGHT`, x: 0, y: 0 }]
let  solutionTwoEntryPoints  =  [
...
console.log({solutionTwoEntryPoints}); 

const  getDataofEntryPoint  =  (entryPoint) => {
    // clone original elements for simulation run
    let  simulationElements  =  JSON.parse(JSON.stringify([...elements])); 

    // utility functions
    const  getCurrentTileInfo  =  ({x, y}) => simulationElements.find((elem) => elem.x === x && elem.y === y);  
    ...
    let  engergizedTiles  =  simulationElements.filter((element) => element.energized); 
    return engergizedTiles.length;
}

console.log({
    solutionOne: Math.max(...solutionOneEntryPoints.map((entryPoint) => getDataofEntryPoint(entryPoint))),
    solutionTwo: Math.max(...solutionTwoEntryPoints.map((entryPoint) => getDataofEntryPoint(entryPoint))),
})
